...
In the following sections, we will first provide an overview of the Gillespie Algorithm and its historical development in the [Section 2](#sec-Topic-Review). Next, the theoretical underpinnings of the algorithm, including its basis in the master equation and stochastic modeling principles, will be detailed in the [Section 3](#sec-Theory). We will then demonstrate the application of the Gillespie Algorithm to two molecular systems in the [Section 4](#sec-application): the photodimerization of thymine, a DNA damage process induced by UV radiation (@MartinezFernandez2022), and the stochastic decay of radioactive isotopes (@Mouret2006). These simulations will be performed using the R programming language (@citeR). Finally, our [Section 5](#sec-conclusion) will summarize the findings, discuss the algorithm's broader implications, and explore potential future applications.
...
Therefore, this simulation involves four species: pyrimidine monomers Thymine 1 (\( T1 \)), Thymine 2 (\( T2 \)), and dimer photoproducts \( CPD \) and \( 64PP \); and two reactions channels: \( c1 \) and \( c2 \).
\[
...
As shown in Figure \ref{fig:dimerization}, both CPD and 64-PP formation follow logarithmic growth, as expected under specific UV radiation according to previously reported quantum yields (@MartinezFernandez2022; @Mouret2006). The number of reactions per unit time plateaus as the pyrimidine pairs decrease, consistent with the propensity functions.
...
Dimerization is widely studied, as it can lead to DNA mutations causing skin cancer.The Gillespie  algorithm allows us to realistically model these reactions, as it allows us to demonstrate that our parameters of interest change over time, rather than following a general logarithmic curve.This algorithm can be further adjusted to show molecular reactions with various molecular species and reaction channels. 
Using the Gillespie algorithm to model molecular reactions, such as the dimerization of thymine shown in this simulation, has wide-ranging implications in fields such as genetics, cancer research, pharmacology, environmental science, and biotechnology. By capturing the inherent randomness in molecular processes, this approach can provide valuable insights into complex biological systems, enabling better prediction of outcomes in disease development, therapy, and environmental impact studies.
...
\appendix
\section{Appendix}
## R Code for [4.1 Photodimerization Simulation](#sec-Photodimerization)

```{r a1, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
# Load required libraries
if (!require("reshape2")) install.packages("reshape2")
if (!require("ggplot2")) install.packages("ggplot2")
library(reshape2)
...
## R code for [4.2 Radioactive Decay Simulation](#sec-Decay)
```{r a2, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
# Define parameters
c <- 0.01  # Decay rate constant
state <- c(R = 10000)  # Initial radioactive substance
max_time <- 1000  # Extended maximum simulation time
