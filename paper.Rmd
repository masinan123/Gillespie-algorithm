...
\[
T1 + T2 \xrightarrow{c1} CPD
\]
\[
T1 + T2 \xrightarrow{c2} 64PP
\]
...
```{r fig-dimerization-sim, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Thymine Photodimerization Simulation"}
...
Both CPD and 64-PP formation follow logarithmic growth, as expected under specific UV radiation according to previously reported quantum yields (@MartinezFernandez2022; @Mouret2006). The number of reactions per unit time plateaus as the pyrimidine pairs decrease, consistent with the propensity functions.
...
- **Dimerization**
As pyrimidine photodimerization transforms two pyrimidine monomers into a dimer, this simulation has accurately represented the formation of two possible dimers, CPD and 64-PP, from pairs of thymine monomers. Dimerization is also known to occur between pairs of cytosine, as well as between thymine and cytosine, with varying rates based on the monomers involved as well as the level of UV radiation (@Mouret2006).
- **Real-World Application**
Dimerization is widely studied, as it can lead to DNA mutations causing skin cancer.The Gillespie  algorithm allows us to realistically model these reactions, as it allows us to demonstrate that our parameters of interest change over time.
This algorithm can be further adjusted to show molecular reactions with various molecular species and reaction channels, with applications in biology, chemistry, and bioinformatics.
...
```{r fig-decay-linear, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Radioactive Decay Simulation in Linear Scale"}
...
```{r fig-decay-log, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Radioactive Decay Simulation in Logarithmic Scale"}
